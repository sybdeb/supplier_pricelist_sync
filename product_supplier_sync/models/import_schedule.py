...
class SupplierImportSchedule(models.Model):
    ...
    def _run_scheduled_import(self):
        ...
        try:
            ...
            # Step 2: NO normalization - use original separator to preserve data integrity
            # BUG FIX: String replacement corrupts CSV data (replaces delimiters inside values)
            # Solution: Pass original separator to wizard, let CSV parser handle it properly
            _logger.info(f"Using original CSV separator: '{self.csv_separator or ","}'")
            ...
            import_wizard = self.env['supplier.direct.import'].with_context(
                ...
                'csv_separator': self.csv_separator or ','  # Use ORIGINAL separator
