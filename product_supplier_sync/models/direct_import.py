...
class DirectImport(models.TransientModel):
            ...
            for idx, header in enumerate(headers):
                ...
                if template and template.mapping_line_ids:
                    ...
                    if template_line:
                        ...
                        is_required = getattr(template_line[0], 'required', False)
                        already_mapped_fields.add(odoo_field)  # Mark this field as used
                        _logger.info(f"Loaded mapping from template: {header} -> {odoo_field} (required={is_required})")
    ...
    def _get_required_fields_map(self):
        """
        Get mapping of required fields from mapping lines
        Returns dict: {odoo_field: csv_column} for all required fields
        """
        required_map = {}
        if self.mapping_lines:
            for line in self.mapping_lines:
                if line.odoo_field and getattr(line, 'required', False):
                    required_map[line.odoo_field] = line.csv_column
        return required_map
    
    def _check_required_fields(self, row, row_num, mapping, required_fields_map):
        """
        Check if all required fields are filled in the CSV row
        Returns: (is_valid, error_dict) - error_dict is None if valid
        """
        if not required_fields_map:
            return True, None
        
        missing_fields = []
        for odoo_field, csv_column in required_fields_map.items():
            value = row.get(csv_column, '').strip()
            if not value:
                missing_fields.append(f"'{csv_column}'")
        
        if missing_fields:
            # Extract product description for error message
            product_description = ''
            for csv_col, odoo_field in mapping.items():
                if odoo_field in ['product.name', 'product.description_purchase', 'supplierinfo.product_name']:
                    if csv_col in row and row[csv_col]:
                        product_description = row[csv_col].strip()
                        break
            
            # Fallback to common column names
            if not product_description:
                for col_name in ['omschrijving', 'description', 'name', 'product_name', 'beschrijving']:
                    if col_name in row and row[col_name]:
                        product_description = row[col_name].strip()
                        break
            
            error_dict = {
                'row': row_num,
                'barcode': row.get(next((k for k, v in mapping.items() if v == 'product.barcode'), ''), '').strip(),
                'product_code': row.get(next((k for k, v in mapping.items() if v == 'product.default_code'), ''), '').strip(),
                'product_name': product_description,
                'brand': '',
                'row_data': row,
                'error': f"Verplichte velden ontbreken: {', '.join(missing_fields)}"
            }
    ...
    def _prescan_csv_and_prepare(self, mapping):
        ...
        supplier_code_col = next((k for k, v in mapping.items() if v == 'supplierinfo.product_code'), None)
        ...
        # Get required fields mapping once (outside loop for performance)
        required_fields_map = self._get_required_fields_map()
        
        # Second pass: categorize rows
        for row_num, row in rows_list:
            try:
                # Check required fields FIRST (before any processing)
                is_valid, error_dict = self._check_required_fields(row, row_num, mapping, required_fields_map)
                if not is_valid:
                    prescan_data['error_rows'].append(error_dict)
                    continue
                ...
                supplier_code = row.get(supplier_code_col, '').strip() if supplier_code_col else None

                # Skip row only if ALL identification fields are empty
                if not barcode and not product_code and not supplier_code:
                    # Extract product name and brand for error logging BEFORE skipping
                    error_product_name = ''
                    error_brand = ''
                    for csv_col, odoo_field in mapping.items():
                        if odoo_field == 'product.name' and csv_col in row and row[csv_col]:
                            error_product_name = row[csv_col].strip()
                        if 'brand' in odoo_field.lower() and csv_col in row and row[csv_col]:
                            error_brand = row[csv_col].strip()
                    
                    prescan_data['error_rows'].append({
                        'row': row_num,
                        'barcode': '',
                        'product_code': '',
                        'product_name': error_product_name,
                        'brand': error_brand,
                        'row_data': row,
...
class DirectImportMappingLine(models.TransientModel):
    ...
    required = fields.Boolean(
        string='Verplicht',
        default=False,
        help="Dit veld moet gevuld zijn, anders wordt de rij overgeslagen"
    )
